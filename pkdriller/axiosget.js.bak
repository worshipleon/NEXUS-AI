...
zokou({
  ...
  try {
    ...
    const replyText = "*Wait Boniphace Md is getting your pair code 🍼✅...*";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://boniphacetech.onrender.com/code?number=${encodedNumber}`;
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "BVONIPHACE-MD ELEMENT INFORMATION",
...
zokou({
  ...
  if (!githubUsername) {
    return repondre("Give me a valid GitHub username like: github boniphace478");
  }
